      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex gap-4 mb-8 justify-center">
          <button onClick={() => setActiveTab('services')}
            className={\`px-6 py-3 rounded-full transition-all \${activeTab === 'services' ? 'bg-white/20 text-white' : 'bg-white/10 text-gray-400 hover:bg-white/15'}\`}>
            🎵 Services
          </button>
          <button onClick={() => setActiveTab('portrait')}
            className={\`px-6 py-3 rounded-full transition-all \${activeTab === 'portrait' ? 'bg-white/20 text-white' : 'bg-white/10 text-gray-400 hover:bg-white/15'}\`}>
            🎨 Music Portrait
          </button>
          <button onClick={() => setActiveTab('stats')}
            className={\`px-6 py-3 rounded-full transition-all \${activeTab === 'stats' ? 'bg-white/20 text-white' : 'bg-white/10 text-gray-400 hover:bg-white/15'}\`}>
            📊 Statistics
          </button>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-3xl p-8">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-black/30 rounded-xl p-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-green-500 rounded-full flex items-center justify-center"><span className="text-2xl">🎵</span></div>
                  <div>
                ...
                {spotifyUser ? (
                  ...
                  <button onClick={connectSpotify} disabled={loadingStates.spotify}
                    className={\`px-6 py-2 rounded-full transition-all \${loadingStates.spotify ? "bg-gray-600 text-gray-300 cursor-wait" : "bg-green-500 hover:bg-green-600 text-white"}\`}>
              ...
              <div className="bg-black/30 rounded-xl p-6 flex items-center justify-between opacity-75">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-pink-500 rounded-full flex items-center justify-center"><span className="text-2xl">🎵</span></div>
                  <div><h3 className="text-white font-bold">Apple Music</h3><p className="text-gray-400 text-sm">Coming soon</p></div>
                </div>
                <button onClick={connectApple} className="bg-gray-500 text-white px-6 py-2 rounded-full cursor-not-allowed" disabled>Connect</button>
              </div>

              <div className="bg-black/30 rounded-xl p-6 flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-red-600 rounded-full flex items-center justify-center"><span className="text-2xl">📊</span></div>
                  <div><h3 className="text-white font-bold">Last.fm</h3><p className="text-gray-400 text-sm">Track your listening history</p></div>
                </div>
                <button onClick={connectLastfm} disabled={loadingStates.lastfm}
                  className={\`px-6 py-2 rounded-full transition-all \${loadingStates.lastfm ? "bg-gray-600 text-gray-300 cursor-wait" : "bg-red-600 hover:bg-red-700 text-white"}\`}>
          ...
          {activeTab === 'portrait' && (
            <div>
              ...
              {spotifyUser ? <MusicPortrait userId={displayWorldId} /> : (
                <div className="text-center py-8">
                  <p className="text-gray-400 mb-4">Connect Spotify to see your music portrait</p>
                  <button onClick={() => setActiveTab('services')}
                    className="bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-full">Go to Services</button>
                </div>
              )}
            </div>
          )}
...
export default function ProfilePage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-gradient-to-b from-purple-900 to-black flex items-center justify-center"><div className="text-white">Loading...</div></div>}>
      <ProfileContent />
    </Suspense>
  );
}
