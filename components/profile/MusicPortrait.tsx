...
export default function MusicPortrait() {
  ...
  const analyzeMusic = async () => {
    ...
    try {
      ...
      const testProfile: MusicProfile = {
        topGenres: ['Pop', 'Electronic', 'Rock', 'Indie', 'Alternative'],
        musicPersonality: 'Eclectic Explorer 🌐',
        energyLevel: 0.75,
        diversityScore: 0.85,
        topArtists: [
          { name: 'Artist 1', genres: ['Pop'], popularity: 90 },
          { name: 'Artist 2', genres: ['Rock'], popularity: 85 },
          { name: 'Artist 3', genres: ['Electronic'], popularity: 80 }
        ],
        topTracks: [
          { name: 'Track 1', artists: [{ name: 'Artist 1' }] },
          { name: 'Track 2', artists: [{ name: 'Artist 2' }] },
          { name: 'Track 3', artists: [{ name: 'Artist 3' }] }
        ],
        sources: ['Spotify', 'Last.fm', 'Apple Music']
      };

      setProfile(testProfile);
      localStorage.setItem('music_profile_cache', JSON.stringify(testProfile));
    } catch (err) {
      setError('Failed to analyze music profile');
    } finally {
      setLoading(false);
    }
  };
